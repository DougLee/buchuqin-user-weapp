...
/**
 * 抽奖大转盘（IKD6FB）：小程序原生页（grilling 拍板，非 web-view H5）。
...
const CHANCE_HINT = "每日 1 次 · 次日 0 点刷新";
...
<template>
  <view class="wheel-page">
    ...
    <view class="wheel-page__head">
      ...
      <text class="wheel-page__pill">{{ headPill }}</text>
    </view>
    <view v-if="loading" class="wheel-page__loading">加载中...</view>
    <template v-else>
      ...
      <view class="wheel-chance">
        ...
        <template v-else-if="drawnToday"
          >今日机会已用完，明天 0 点再来</template
        >
    ...
    <view
      ...
      @tap="closeResult"
    >
      <view class="wheel-pop" :class="{ 'wheel-pop--green': result.type === 'coupon' }" @tap.stop>
